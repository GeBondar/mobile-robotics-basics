...
Actions используют модель клиент-сервер, аналогичную модели publisher-subscriber. Нода-клиент action отправляет цель ноде-серверу action, которая подтверждает цель и возвращает поток сообщений обратной связи и результат.

![Модель действия](images/Action-SingleActionClient.gif)

## Предварительные требования

Этот урок опирается на концепции нод и топиков, рассмотренные в предыдущих уроках.
...
При запуске ноды `/teleop_turtle` вы увидите следующее сообщение в терминале:
...
Сосредоточимся на второй строке, которая соответствует action. Первая инструкция относится к топику `cmd_vel`, обсуждавшемуся ранее в уроке по топикам.
...
Обратите внимание на терминал, где запущена нода `/turtlesim`. Каждый раз, когда вы нажимаете одну из этих клавиш, вы отправляете цель action-серверу, который является частью ноды `/turtlesim`. Цель состоит в том, чтобы повернуть черепаху в заданном направлении. После завершения поворота должно отобразиться сообщение с результатом:
...
Попробуйте нажать `C`, а затем `F` до завершения поворота. В терминале с нодой `/turtlesim` вы увидите сообщение:
...
Чтобы увидеть список actions, которые предоставляет нода (в данном случае `/turtlesim`), откройте новый терминал и выполните команду:
...
Обратите внимание, что `/turtle1/rotate_absolute` находится в разделе **Action Servers**. Это означает, что нода `/turtlesim` отвечает на этот action и предоставляет обратную связь.

Нода `/teleop_turtle` имеет имя `/turtle1/rotate_absolute` в разделе **Action Clients**, что означает, что она отправляет цели для этого action. Чтобы убедиться в этом, выполните:
...
Это единственный action в текущем графе ROS. Он управляет поворотом черепахи, как вы уже видели. Из предыдущего шага мы знаем, что для него есть один клиент (в ноде `/teleop_turtle`) и один сервер (в ноде `/turtlesim`).

#### 4.1 ros2 action list -t

Actions имеют типы, аналогично топикам и сервисам. Чтобы узнать тип action `/turtle1/rotate_absolute`, выполните:
...
Это подтверждает, что нода `/teleop_turtle` является клиентом, а `/turtlesim` — сервером для данного action.
...
* Официальная документация ROS 2 Jazzy: [Understanding actions](https://docs.ros.org/en/jazzy/Tutorials/Beginner-CLI-Tools/Understanding-ROS2-Actions/Understanding-ROS2-Actions.html) (на английском)
