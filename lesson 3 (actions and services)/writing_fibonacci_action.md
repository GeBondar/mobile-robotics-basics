# Создание action-сервера Fibonacci в ROS 2 Jazzy

Этот туториал показывает полный цикл создания собственного action-сервера в ROS 2 Jazzy. Вы определите пользовательский тип action, реализуете сервер для вычисления последовательности Фибоначчи и напишете клиент для взаимодействия с ним.
...
```
Этот пакет создается как CMake-пакет, так как требует кодогенерации, но созданные интерфейсы можно будет использовать как в C++, так и в Python-нодах.
...
if __name__ == '__main__':
    main()
```
*   `ActionServer` создается с указанием ноды (`self`), типа action (`Fibonacci`), имени action (`'fibonacci'`) и функции обратного вызова `execute_callback`, которая будет вызываться для каждой принятой цели.
...
```python
import time  # Добавьте рядом с остальными импортами
