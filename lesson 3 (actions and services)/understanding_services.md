...
Сервисы — это ещё один способ коммуникации между нодами в графе ROS. Сервисы основаны на модели «запрос–ответ» (call-and-response) в отличие от модели publisher-subscriber для топиков. В то время как топики позволяют нодам подписываться на потоки данных и получать постоянные обновления, сервисы предоставляют данные только тогда, когда они явно вызваны клиентом.
...
Некоторые концепции, упомянутые в этом уроке, такие как ноды и топики, были рассмотрены в предыдущих уроках.
...
Вы увидите, что обе ноды имеют одни и те же шесть сервисов с параметрами в названиях. Почти каждая нода в ROS 2 имеет эти инфраструктурные сервисы, на которых построены параметры. Подробнее о параметрах будет рассказано в следующем уроке. В этом уроке мы не будем рассматривать сервисы, связанные с параметрами.
...
Запустите демонстрационные ноды с поддержкой интроспекции:
...
Ноды могут обмениваться данными с помощью сервисов в ROS 2. В отличие от топиков — однонаправленного потока, где нода публикует информацию, потребляемую одним или несколькими подписчиками, — сервис представляет собой шаблон «запрос/ответ»: клиент отправляет запрос ноде, предоставляющей сервис, а сервис обрабатывает запрос и формирует ответ.

Обычно не рекомендуется использовать сервисы для непрерывных вызовов; для этого лучше подходят топики или actions.

В этом уроке вы использовали инструменты командной строки для идентификации, изучения и вызова сервисов.

## Следующие шаги

В следующем уроке «Понимание параметров» вы узнаете о настройке параметров нод.
