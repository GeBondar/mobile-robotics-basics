...
## Цель
Создать собственный сервис в ROS 2, который позволит получать текущие координаты черепахи из симулятора turtlesim. Вы научитесь описывать тип сервиса, реализовывать сервер и клиент на Python, а также комбинировать топики и сервисы в одной ноде.
...
1. Создадим два ROS 2 пакета:
   ...
   - `turtle_pose_service` — пакет типа `ament_python` для Python-нод (сервер и клиент).
...
> **Важно:** `rosidl_default_generators` указывается именно как `buildtool_depend`, а не `depend`. Это инструмент сборки, который не нужен во время выполнения.
...
- Нода подписывается на топик `/turtle1/pose` и в коллбэке `pose_callback` сохраняет последнее сообщение в `self.current_pose`.
...
### **5.3 Сборка пакетов**
Сначала соберём пакет интерфейсов, затем пакет с нодами:
```bash
cd ~/ros2_ws
...
### **Упражнение 2: Многосервисная нода**
Объедините оба сервиса (`GetPose` и `RandomTeleport`) в одной ноде. Проверьте их работу.
...
- **Разделение интерфейсов и логики нод на два пакета**
...
2. **Работа с параметрами ноды** для настройки поведения без перекомпиляции.
3. **Создание более сложных сервисов** с непустыми запросами (например, задание целевой позиции).
4. **Интеграция сервисов в большие проекты** с несколькими нодами.
