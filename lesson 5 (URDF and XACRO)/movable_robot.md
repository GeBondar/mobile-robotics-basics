...
## Практическая часть: подвижная модель R2D2
...
Разберём фрагменты URDF, которые отвечают за подвижные соединения модели.
...
3.  Нода **`robot_state_publisher`** подписана на `/joint_states`. Она знает URDF-модель и, получая положения суставов, вычисляет, где в пространстве должен находиться каждый линк.
...
- `joint_state_publisher_gui` и `robot_state_publisher` превращают статическую модель в интерактивную, публикуя трансформации в tf2.
